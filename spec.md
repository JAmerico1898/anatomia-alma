...
| Objeto 3D | Figura humana translúcida no centro de cascas concêntricas do microcosmo. |
...
|---|---|---|---|---|
| 1 | `cascas` | Cascas do microcosmo | violeta | As quatro esferas concêntricas do sistema de vida (Glossário, p. 373) |
...
Visíveis por padrão: todos exceto `cascas` (que começa em modo *silhueta*, para
...
#### Sistema 1 · Cascas do microcosmo (4)
...
| `campo-magnetico-septuplo` | Campo espiritual magnético sétuplo | casca externa do microcosmo | Sintonizado com o campo eletromagnético da terra; mantém o sistema preso | Sintonizado com o campo da Fraternidade Universal; atrai materiais de construção não desta natureza | 1 | Gloss. p. 373; I-6 pp. 72–73 |
...
matemático do microcosmo". As cascas concêntricas são, portanto, centradas na rosa,
...
app/scene/
  ...
  shells.ts              4 cascas: IcosahedronGeometry + ShaderMaterial fresnel
...
|---|---|---|
| `campo-magnetico-septuplo` | `[0,0,0]` | casca r=2.00 |
| `ser-aural` | `[0,0,0]` | casca r=1.55 |
| `campo-de-respiracao` | `[0,0,0]` | casca r=1.15 |
...
superfície interna da casca `ser-aural`.
...
|---|---|
| Painel `Sistemas` | Switch por sistema; contagem de estruturas; clique no nome isola aquele sistema. Presets: **Tudo**, **Só a figura**, **Só as cascas**, **Só as correntes**. |
| Slider `Separar camadas` | 0–100%. Afasta as quatro cascas radialmente e depois separa os grupos de órgãos. Substitui o "Explode anatomy". |
...
|---|---|---|---|
| `cascas` | violeta | `#8a7fb0` | manto dos reis-sacerdotes |
...
Figura, cascas, coluna, simpático, focos, rótulos. Órbita, zoom, seleção por
...
- **Cascas cortavam a figura.** Com os eixos do §6.1 (pés em `y=-1.30`), uma
  casca de raio 1.15 centrada na rosa deixaria as pernas fora do campo de
